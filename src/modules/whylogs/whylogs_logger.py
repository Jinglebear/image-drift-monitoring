...
class Whylogs_Logger():
    ...
    def create_output_dir(self, path: str) -> None:
        ...
        pathlib.Path(
            path + self.PROFILE_SUMMARIES).mkdir(parents=True, exist_ok=True)
        pathlib.Path(
            path + self.PROFILE_COMPARE).mkdir(parents=True, exist_ok=True)
    ...
    def log_basic_image_data_from_dir(self, data_directory_path: str, sub_dir_path: str, batch_size: int = 0, shuffle: bool = False) -> DatasetProfileView:
        try:
            ...
            self.logger.info(
                'Started logging image data from: {}'.format(sub_dir_path))
            profile = None
            ...
            for img_name in progressbar(data_directory_list):
                ...
                _.set_dataset_timestamp(self.my_datetime)
            ...
            self.logger.info('Logged image data from: {} with batch size {}'.format(sub_dir_path, profile.to_pandas()[
                             'image/Brightness.mean:counts/n']['image']))  # get batchsize from output data
            return profile
        ...
        try:
            ...
            for img in progressbar(pil_data_arr):
                ...
                if profile is None:
                    ...
                    # merge each profile while looping
                    profile = profile.merge(_view)
            self.logger.info('Logged image data from PIL image data with batch size {}'.format(
                profile.to_pandas()['image/Brightness.mean:counts/n']['image']))  # get batchsize from output data
            return profile
        except Exception as e:
            self.logger.exception('Exception in log_data(): {}'.format(e))
    ...
    def log_pil_image_to_profile(self, image: PIL_IMAGE, profile: DatasetProfileView) -> DatasetProfileView:
        try:
            ...
            self.logger.exception(
                'Exception in log_pil_image_to_profile: {}'.format(e))
    ...
    def serialize_profile(self, profile: DatasetProfileView, binary_name: str, data_directory_path: str) -> str:
        try:
            serialize_profile_bin_path = '{}{}/{}.bin'.format(
                data_directory_path, self.OUTPUT_BIN, binary_name)
            profile.write(serialize_profile_bin_path)
            return serialize_profile_bin_path
        except Exception as e:
            self.logger.exception(
                'Exception in serialize_profile(): {}'.format(e))
    ...
    def deserialize_profile(self, data_directory_path: str, binary_name: str) -> DatasetProfileView:
        try:
            ...
            self.logger.exception(
                'Exception in deserialize_profile(): {}'.format(e))
    ...
    def create_profile_summary_json(self, profile: DatasetProfileView, data_directory_path: str, summary_name: str) -> str:
        try:
            profile_summary_json_path = '{}{}/{}.json'.format(
                data_directory_path, self.PROFILE_SUMMARIES, summary_name)
            ...
            self.logger.info(
                'Created profile summary json for {}'.format(summary_name))
            return profile_summary_json_path
        except Exception as e:
            self.logger.exception(
                'Exception in create_profile_summary(): {}'.format(e))
    ...
    def create_profile_compare_summary_json(self, target_profile: DatasetProfileView, ref_profile: DatasetProfileView, data_directory_path: str, compare_summary_name: str) -> str:
        try:
            compare_summary_json_path = '{}{}/{}.json'.format(
                data_directory_path, self.PROFILE_COMPARE, compare_summary_name)
    ...
    def create_drift_metric_df_from_comp_summary_json(self, data_directory_path: str, compare_summary_name: str) -> pd.DataFrame:
        try:
            ...
            df.to_csv('{}{}/{}.csv'.format(data_directory_path,
                      self.PROFILE_COMPARE, compare_summary_name))
            return df
        except Exception as e:
            self.logger.exception(
    ...
    def create_visualization(self, data_directory_path: str, viz_name: str, target_profile: DatasetProfileView, referece_profile: DatasetProfileView) -> str:
        try:
            ...
            html_viz_path = '{}{}/{}.html'.format(
                data_directory_path, self.PROFILE_COMPARE, viz_name)
            with open(html_viz_path, 'w') as output:
                output.write(html_viz.data)
            return html_viz_path
        except Exception as e:
            self.logger.exception(
                'Exception in create_metrics(): {}'.format(e))
