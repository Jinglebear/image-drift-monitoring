...
CAMELYON_ROOT_PATH ='/home/jinglewsl/evoila/sandbox/whylogs_v1/ml-image-drift-monitoring/src/modules/whylogs/data'
...
def plot_difference(percentage : int, dfs : List(pd.DataFrame),output_path:str, target: str,ref:str, ref_p :int):
    df_joined = dfs[0]
    for i in range(1,20,1):
        df_joined = df_joined.join(dfs[i].set_index('metric'),on='metric')
    data= {
    'metric' :           [ str(out) for out in range(5,100,5)],
    ...
    'Drift Threshold' :  np.full(19,0.05),
    }
    ...
    ax =converted_df.plot.line(
        ...
        x='metric',
        figsize=(12,6), 
        title='{} vs {} {}'.format(ref,target, percentage),
    ...
    plt.savefig( '{}{}_{}_v_{}_{}.png'.format(output_path,ref,ref_p,target,percentage))
...
def log_compare(target,ref,data_directory_path,compare_summary_name):
    ...
    w_logger.create_drift_metric_df_from_comp_summary_json(data_directory_path=data_directory_path,compare_summary_name=compare_summary_name)
...
def log_percentage_of_dataset(percentage: int, pil_images : List[Image], size : int, split: str):
    ...
    w_logger.serialize_profile(profile=tmp_profile,binary_name='{}_camelyon_profile_{}'.format(split,percentage),data_directory_path=CAMELYON_ROOT_PATH)
