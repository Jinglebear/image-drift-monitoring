...
def log_profile_to_bin_multiple_processes(w_logger: Whylogs_Logger, percentage: int, indices: List[int], dataset: any, num_processes: int, split: str, dataset_name: str, dataset_dir_path: str):
    ...
    # calculate logging amout from percentage
    ...
    # split array into x arrays for x processes
    split_array = np.array_split(indices_arr, num_processes)
    ...
    for i in range(num_processes):
        tmp_process = Process(target=logging_task, args=(
            split_array[i], queue, dataset, w_logger))
    ...
    merged_profiles = w_logger.merge_profiles(
        profiles=profiles)  # merge all profiles from processes
    print('merged Profiles')

    print(w_logger.serialize_profile(profile=merged_profiles, binary_name='{}_{}_profile_{}'.format(
        split, dataset_name, percentage), data_directory_path=dataset_dir_path))  # serialize the merged profiles to output bin


# needed for logging (multiprocess)
def logging_task(indices: any, queue: Queue, dataset: any, w_logger: Whylogs_Logger):
    first_profile = w_logger.log_pil_image_to_profile(
        image=dataset[indices[0]][0], profile=None)

    for idx in indices[1:]:
        first_profile = w_logger.log_pil_image_to_profile(
            image=dataset[idx][0], profile=first_profile)
