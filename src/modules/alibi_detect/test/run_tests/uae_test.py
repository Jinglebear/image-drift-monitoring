...
def main():
    ...
    for name in ['rxrx1', 'iwildcam', 'globalwheat', 'camelyon']:
        ...
        if name == 'globalwheat':
            drift_detection_config["PATHS"]["DETECTOR_DIR_PATH"] = '/home/ubuntu/image-drift-monitoring/config/detectors/GlobalWheat/UAE_n_32'
        ...
        if name == 'camelyon':
            drift_detection_config["PATHS"]["DETECTOR_DIR_PATH"] = '/home/ubuntu/image-drift-monitoring/config/detectors/Camelyon/UAE_n_32'
            ...
            df_new = pd.DataFrame(
                data, index=["{}".format(i) for i in range(1, 21, 1)])

            for i in ['KS', 'CVM', 'MMD', 'LSDD']:
                t = timer()
                myUAE = UntrainedAutoencoder()
                myUAE.import_detector(path='{}/{}_uae_{}'.format(
                ...
                df_new.to_excel(
                    '{}_uae_{}_results_recurring.xlsx'.format(DATASET_NAME, i))
        ...
        df_new = pd.DataFrame(
            data, index=["{}".format(i) for i in range(5, 105, 5)])
        ...
        for i in ['KS', 'CVM', 'MMD', 'LSDD']:
            ...
            df_new.to_excel(
                '{}_uae_{}_results_incremental.xlsx'.format(DATASET_NAME, i))
        ...
        df_new = pd.DataFrame(
            data, index=["{}".format(i) for i in range(10, 210, 10)])
        ...
        for i in ['KS', 'CVM', 'MMD', 'LSDD']:
            ...
            df_new.to_excel(
                '{}_uae_{}_results_sudden.xlsx'.format(DATASET_NAME, i))
